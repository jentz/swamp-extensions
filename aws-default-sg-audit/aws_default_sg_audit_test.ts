/**
 * Colocated publication-sanity tests for `@jentz/aws-default-sg-audit`.
 *
...
import { model } from "./aws_default_sg_audit.ts";
...
Deno.test("model metadata: entrypoint stays in sync with manifest", async () => {
  ...
  assertEquals(Object.keys(model.resources).sort(), ["finding", "scan_error"]);
