...
| Every finding is `skip`                      | Step ran but its data was missing or unparseable.                                                                                                                             | Read the report's JSON `findings[].message` for the per-rule reason (e.g. "bucket data file missing or unparseable", "did not match expected shape"); cross-check the workflow logs for the upstream step's status and verify the upstream extension version. |
