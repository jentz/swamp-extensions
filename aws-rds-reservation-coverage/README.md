...
| `errorsByKind` | Scan errors counted by `network` / `auth_expired` / `access_denied` / `other`. `network` rows (transient DNS/socket failures) are surfaced in their own report section and counted here, not folded into `other`. |
