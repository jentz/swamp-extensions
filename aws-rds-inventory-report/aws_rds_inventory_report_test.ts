/**
 * Colocated smoke tests for the @jentz/aws-rds-inventory-report extension.
 *
 * This file sits beside the report entrypoint (`aws_rds_inventory_report.ts`)
 ...
 * `tests/aws_rds_inventory_report_test.ts`. This file deliberately does NOT
 * duplicate that matrix.
 */

import { assert, assertEquals, assertExists } from "jsr:@std/assert@1";

import { report } from "./aws_rds_inventory_report.ts";
...
Deno.test(
  ...
  async () => {
    ...
    assert(out.markdown.includes("AWS RDS Inventory"));
    assertEquals(out.json.report, "@jentz/aws-rds-inventory-report");
    assertEquals(out.json.workflow, "smoke-workflow");
    // No clusters or instances were found, so the counts are empty.
    assertEquals(out.json.clusterCount, 0);
    assertEquals(out.json.instanceCount, 0);
