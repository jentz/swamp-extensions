...
read-only-recon workflows. Each subdirectory is a self-contained
publishable package with its own manifest, source, tests, and README.
...
The repo is a swamp repository in its own right (`.swamp.yaml` at the
root). Tests run under the swamp-bundled Deno so versions and resolver
behavior match what `swamp extension push` uses:
...
Each manifest opts in to `paths.base: manifest` so per-extension
subdirectories are self-contained: manifest, source, README, and
LICENSE all sit alongside each other.

To validate a manifest and produce a publishable tarball locally
without uploading:
...
Bug reports, feature requests, security reports, and ripples (comments)
go through `swamp issue` and are routed to this repo's issue tracker:
...
(`YYYY.MM.DD.MICRO`). The list of breaking-change rules is documented
in each extension's README.
