/**
 * Colocated publication-sanity tests for `@jentz/aws-stackset-lifecycle`.
 *
...
import { model } from "./aws_stackset_lifecycle.ts";
...
Deno.test("model metadata: entrypoint stays in sync with manifest", async () => {
  ...
  assertEquals(Object.keys(model.resources).sort(), ["result"]);
  // Mutating surface: exactly the two retirement methods, in declaration order.
  assertEquals(Object.keys(model.methods), [
    "deleteInstances",
    "deleteStackSet",
  ]);
});
